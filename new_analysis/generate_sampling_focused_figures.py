...
def create_sampling_method_bias_analysis(data, output_dir="outputs/sampling_focused"):
    ...
    # 使用专业科研配色 - Nature/Science风格
    # CETraS: 深蓝紫色，RWFB: 深红橙色
    color_cetras = '#3B4CC0'  # 深蓝紫 (专业、权威)
    color_rwfb = '#B40426'    # 深红 (醒目、对比强)
    
    # 根据偏差方向着色，并根据强度调整透明度
    max_bias = max(abs(b) for b in bias_values)
    bar_colors = []
    alphas = []
    
    for b in bias_values:
        if b > 0:
            bar_colors.append(color_cetras)
        else:
            bar_colors.append(color_rwfb)
        # 透明度与偏差大小成正比
        alphas.append(0.5 + 0.5 * (abs(b) / max_bias))
    
    bars = ax.bar(role_names, bias_values, color=bar_colors, 
    ...
    # 添加零线
    ax.axhline(y=0, color='#2D2D2D', linestyle='-', linewidth=1.5, alpha=0.9)
    
    # 设置标签
    ax.set_xlabel('Bitcoin Address Role Categories', fontweight='bold', fontsize=13)
    ax.set_ylabel('Sampling Bias (CETraS - RWFB)', fontweight='bold', fontsize=13)
    ...
    # 精致网格
    ax.grid(True, alpha=0.25, axis='y', linestyle='--', linewidth=0.7, color='gray')
    ...
    # 添加专业图例
    legend_elements = [
        plt.Rectangle((0,0),1,1, color=color_cetras, alpha=0.85, 
                     edgecolor='black', linewidth=0.8, label='CETraS Preferred'),
        plt.Rectangle((0,0),1,1, color=color_rwfb, alpha=0.85, 
                     edgecolor='black', linewidth=0.8, label='RWFB Preferred')
    ]
