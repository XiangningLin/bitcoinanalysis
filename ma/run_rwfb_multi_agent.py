...
async def main():
    ...
    nodes_blob = read_text_limited(os.path.join(BASE_DIR, "llm4tg_nodes_top100_rwfb.jsonl"), 30000)
    edges_blob = read_text_limited(os.path.join(BASE_DIR, "llm4tg_edges_top100_rwfb.csv"), 20000)
    ...
    t1 = Task(task_id="rwfb-roles", name="RWFB Role Classification", description="Classify roles",
              required_capability="role_classification",
              payload={"nodes_blob": nodes_blob, "edges_blob": edges_blob, "base_prompt": prompts["p1"], "tag": "RWFB"},
              priority=TaskPriority.MEDIUM)
    t2 = Task(task_id="rwfb-anomaly", name="RWFB Anomaly Analysis", description="Explain patterns",
              required_capability="anomaly_analysis",
              payload={"nodes_blob": nodes_blob, "edges_blob": edges_blob, "base_prompt": prompts["p2"], "tag": "RWFB"},
              priority=TaskPriority.MEDIUM)
    t3 = Task(task_id="rwfb-decent", name="RWFB Decentralization Summary", description="Summarize",
              required_capability="decentralization_summary",
              payload={"nodes_blob": nodes_blob, "edges_blob": edges_blob, "base_prompt": prompts["p3"], "tag": "RWFB"},
              priority=TaskPriority.MEDIUM)
    ...
    s1 = await wait_done("rwfb-roles")
    s2 = await wait_done("rwfb-anomaly")
    s3 = await wait_done("rwfb-decent")

    # Prepare output directory under ma/: outputs/rwfb
    out_dir = os.path.join(CURRENT_DIR, "outputs", "rwfb")
    ...
    if s1 and s1.get("result"):
        ...
        items = r.get("items", [])
        raw = r.get("raw", "")
        json.dump(items, open(os.path.join(out_dir, "rwfb_role_predictions.json"),"w",encoding="utf-8"), ensure_ascii=False, indent=2)
        open(os.path.join(out_dir, "rwfb_role_predictions.part1.raw.txt"),"w",encoding="utf-8").write(raw)
    if s2 and s2.get("result"):
        open(os.path.join(out_dir, "rwfb_anomaly_explained.txt"),"w",encoding="utf-8").write(s2["result"].get("text",""))
    if s3 and s3.get("result"):
        open(os.path.join(out_dir, "rwfb_decentralization_summary.txt"),"w",encoding="utf-8").write(s3["result"].get("text",""))
